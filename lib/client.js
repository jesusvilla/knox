...
const debug = require('debug')('knox');
...
const BUCKET_OPS_MAX = 1000;

// http://docs.aws.amazon.com/AmazonS3/latest/dev/BucketRestrictions.html
const MIN_BUCKET_LENGTH = 3;
const MAX_NON_US_STANDARD_BUCKET_LENGTH = 63;
const MAX_US_STANDARD_BUCKET_LENGTH = 255;
const US_STANDARD_BUCKET = /^[A-Za-z0-9\._-]*$/;
const BUCKET_LABEL = /^(?:[a-z0-9][a-z0-9-]*[a-z0-9]|[a-z0-9])$/;
...
Client.prototype.putStream = function(stream, filename, headers, fn){
  ...
  const req = this.put(filename, headers);

  fn = utils.once(fn);
  registerReqListeners(req, fn);
  stream.on('error', fn);

  utils.streamCounter(stream, (bytes) => {
    req.emit('progress', {
      percent: bytes / contentLength * 100 | 0,
      written: bytes,
      total: contentLength
    });    
  });
...
Client.prototype.deleteMultiple = function(filenames, headers, fn){
  ...
  headers['Content-MD5'] = utils.crypto.md5(xml);
...
Client.prototype.list = function(params, headers, fn){
  ...
  const url = params ? '?' + qs.stringify(params) : '';
  const req = this.request('GET', url, headers);
  registerReqListeners(req, (err, res) => {
    ...
    res.on('end', () => {
      utils.xml2js(xmlStr, (error, data) => {
        if (err) return fn(err);
